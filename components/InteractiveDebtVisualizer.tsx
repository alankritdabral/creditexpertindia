...
export function InteractiveDebtVisualizer({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  // PHASES:
  // 0.0 - 0.2: Hold Initial State
  // 0.2 - 0.4: Collapse Cards
  // 0.4 - 0.6: The Calculation (Numbers scrub)
  ...
  return (
    <motion.div 
      ...
      className="relative w-full max-w-4xl mx-auto px-4 flex items-center justify-center shrink-0 my-2 overflow-hidden h-[340px] sm:h-[420px]"
    >
      ...
      >
        <div className="relative w-full max-w-2xl h-[340px] sm:h-[420px]">
          <div className="absolute inset-x-0 top-4 text-center">
            <h3 className="text-xl font-bold tracking-wider text-warning-red mb-2 uppercase">Your Debt Today</h3>
          </div>

          {/* Card 1 */}
          <motion.div 
            style={{ x: collapseX1, y: collapseY1 }}
            className="absolute left-0 sm:left-12 top-12 sm:top-16 w-36 sm:w-48 bg-white p-4 rounded-2xl shadow-lg border border-red-100 flex flex-col items-center justify-center"
          >
            <p className="text-lg font-bold text-text-main">₹14,500</p>
            <p className="text-xs text-text-muted font-medium">Personal Loan</p>
          </motion.div>

          {/* Card 2 */}
          <motion.div 
            style={{ x: collapseX2, y: collapseY2 }}
            className="absolute right-0 sm:right-12 top-12 sm:top-16 w-36 sm:w-48 bg-white p-4 rounded-2xl shadow-lg border border-red-100 flex flex-col items-center justify-center"
          >
            <p className="text-lg font-bold text-text-main">₹8,500</p>
            <p className="text-xs text-text-muted font-medium">Credit Card</p>
          </motion.div>

          {/* Card 3 */}
          <motion.div 
            style={{ x: collapseX3, y: collapseY3 }}
            className="absolute left-0 sm:left-12 bottom-12 sm:bottom-16 w-36 sm:w-48 bg-white p-4 rounded-2xl shadow-lg border border-red-100 flex flex-col items-center justify-center"
          >
            <p className="text-lg font-bold text-text-main">₹6,000</p>
            <p className="text-xs text-text-muted font-medium">App Loan</p>
          </motion.div>

          {/* Card 4 */}
          <motion.div 
            style={{ x: collapseX4, y: collapseY4 }}
            className="absolute right-0 sm:right-12 bottom-12 sm:bottom-16 w-36 sm:w-48 bg-white p-4 rounded-2xl shadow-lg border border-red-100 flex flex-col items-center justify-center"
          >
            <p className="text-lg font-bold text-text-main">₹9,500</p>
            <p className="text-xs text-text-muted font-medium">Personal Loan</p>
          </motion.div>
          ...
          <div className="absolute inset-x-0 bottom-0 flex justify-between px-12 sm:px-24">
            <div className="text-left">
              <p className="text-xs text-text-muted font-medium uppercase">Monthly outflow</p>
              <p className="text-lg font-bold text-warning-red line-through">₹38,500</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-text-muted font-medium uppercase">Highest rate</p>
              <p className="text-lg font-bold text-warning-red">42% p.a.</p>
            </div>
          </div>
        </div>
      </motion.div>
